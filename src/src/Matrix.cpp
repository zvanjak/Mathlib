...
Matrix  Invert( const Matrix &T )
{
	...
	if( m == n )
	{
		//matricu Ret izjedna�iti sa jedini�nom matricom
		Ret.MakeUnitMatrix();
...
//			if( k != n-1 )
			{
				...
				// u k-tom stupcu na�i najve�i element po apsolutnoj vrijednosti
				for( i=k+1; i<n; i++ )
				{
				...
				if( a1 == 0 )		// zna�i singularna matrica
				{
...
{
	//	Vr�i QR dekompoziciju
	//		Gornja trokutna matrica R se vra�a u gornjem trokutu matrice 'a'
	//		osim dijagonalnih elemenata, koji se vra�aju u vektoru 'd'
	//		Ortogonalan matrica Q se vra�a u faktoriziranoj formi
	int			i, j, k, n;
...
{
	//	Rije�ava sistem pomo�u QR dekompozicije
	//		u 'b' se nalazi vektor desne strane, i u njemu se na kraju nalazi rije�enje sistema
	int		i, j, n;
	double	sum, tau;
...
{
	//	Rije�ava R*x = b, gdje je R gornja trokutna matrica spremljena u 'a' i 'd'
	int			i, j, n;
	double	sum;
